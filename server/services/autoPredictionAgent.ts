...
- Economic uncertainty (bullish for gold)`;
    ...
    const response = await invokeLLM({
      messages: [
        { role: "system", content: "You are an expert gold market analyst. Always respond with valid JSON." },
        ...
        json_schema: {
          ...
          schema: {
            ...
            properties: {
              ...
              sentiment: {
                type: "number",
                description: "Sentiment from -100 to 100"
              },
              summary: {
                type: "string",
              ...
              reasoning: {
                type: "array",
                items: { type: "string" },
                description: "List of key reasons"
              }
            },
            required: ["goldImpact", "impactScore", "sentiment", "summary", "reasoning"],
    ...
    console.error(`[AutoPredictionAgent] Error analyzing article ${article.id}:`, error);
...
export async function generatePredictionFromNews(
  ...
  if (analyses.length === 0) {
    console.log("[AutoPredictionAgent] No analyses provided, skipping prediction");
    return null;
  }

  console.log(`[AutoPredictionAgent] Generating prediction from ${analyses.length} news analyses`);
  ...
  try {
    ...
    const weightedSentiment = analyses.reduce((sum, a) => {
      return sum + (a.sentiment * a.impactScore / 100);
    }, 0) / analyses.length;
    
    // Count bullish vs bearish signals
    const bullishCount = analyses.filter(a => a.goldImpact === "bullish").length;
    const bearishCount = analyses.filter(a => a.goldImpact === "bearish").length;
    ...
    const consensus = totalSignals > 0 ? Math.max(bullishCount, bearishCount) / totalSignals : 0.5;
    ...
    const rangeMin = direction === "bear" ? -baseRange - 10 : direction === "bull" ? 5 : -10;
    const rangeMax = direction === "bear" ? -5 : direction === "bull" ? baseRange + 10 : 10;
    ...
    console.log(`[AutoPredictionAgent] Created prediction ${predictionId}: ${direction} (${confidence}% confidence)`);
...
export async function runAutomatedAnalysis(): Promise<{
  ...
  try {
    ...
    console.log(`[AutoPredictionAgent] Found ${articles.length} unanalyzed articles`);
    ...
    if (highImpactAnalyses.length >= 2) {
      ...
      const prediction = await generatePredictionFromNews(highImpactAnalyses, "1H");
