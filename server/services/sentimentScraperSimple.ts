...
async function scrapeMyFxBook(symbol: string): Promise<SentimentData | null> {
  try {
    ...
    const response = await axios.get(`https://www.myfxbook.com/community/outlook/${symbol}`, {
      headers: {
    ...
    console.log(`[SentimentScraper] Could not extract MyFxBook data for ${symbol}`);
    return null;
  } catch (error) {
...
async function scrapeDukascopy(symbol: string): Promise<SentimentData | null> {
  try {
    ...
    const response = await axios.get('https://www.dukascopy.com/swiss/english/marketwatch/sentiment/', {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
      },
      timeout: 15000
    ...
    console.log(`[SentimentScraper] Could not extract Dukascopy data for ${symbol}`);
    return null;
  } catch (error) {
...
async function scrapeFXSSI(symbol: string): Promise<SentimentData | null> {
  try {
    ...
    console.log(`[SentimentScraper] Could not extract FXSSI data for ${symbol}`);
    return null;
  } catch (error) {
...
export async function fetchAllSentiment(symbol: string): Promise<SentimentData[]> {
  ...
  if (sentimentData.length < 3) {
    console.log(`[SentimentScraper] Only got ${sentimentData.length} real sources, adding mock data`);
