 ...
 */
export async function scrapeMyFxBookCalendar(): Promise<EconomicCalendarEvent[]> {
  try {
    ...
    $('table tr, .calendar-row').each((i, row) => {
      ...
      const impact = $row.find('.impact, td:nth-child(2)').text().trim().toLowerCase();
      ...
      if (title && title.length > 3) {
        events.push({
          ...
          forecast: $row.find('.forecast, td:nth-child(4)').text().trim() || undefined,
          previous: $row.find('.previous, td:nth-child(5)').text().trim() || undefined,
          actual: $row.find('.actual, td:nth-child(6)').text().trim() || undefined,
        });
      }
    });
 ...
 */
export async function scrapeTradingEconomicsCalendar(): Promise<EconomicCalendarEvent[]> {
  try {
    ...
    $('table tbody tr, .calendar-event').each((i, row) => {
      ...
      if (title && title.length > 3) {
        events.push({
          ...
          country: country || undefined,
          eventTime: new Date(),
          forecast: $row.find('td:nth-child(4), .forecast').text().trim() || undefined,
          previous: $row.find('td:nth-child(5), .previous').text().trim() || undefined,
          actual: $row.find('td:nth-child(6), .actual').text().trim() || undefined,
        });
      }
    });
    
    console.log(`[NewsScraper] Found ${events.length} events from Trading Economics`);
    return events.slice(0, 20);
  } catch (error) {
    console.error("[NewsScraper] Error scraping Trading Economics calendar:", error);
    return [];
  }
}
 ...
 */
export async function scrapeReutersGold(sourceId: string): Promise<ScrapedNews[]> {
  try {
    console.log("[NewsScraper] Fetching Reuters Gold News");
    
    const response = await axios.get("https://www.reuters.com/markets/commodities/", {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
      },
      timeout: 15000
    ...
    console.log(`[NewsScraper] Found ${articles.length} gold articles from Reuters`);
 ...
 */
export async function scrapeKitcoNews(sourceId: string): Promise<ScrapedNews[]> {
  try {
 ...
 */
export function generateMockSocialPosts(sourceId: string, sourceName: string): ScrapedNews[] {
  const mockPosts = [
    {
      title: `${sourceName}: Gold prices showing strength amid dollar weakness`,
      content: "The precious metal continues to attract safe-haven demand as geopolitical tensions rise.",
    },
    {
      title: `${sourceName}: Fed policy remains key driver for gold markets`,
      content: "Interest rate decisions will be crucial for gold's trajectory in coming months.",
    },
    {
      title: `${sourceName}: Central banks continue gold accumulation`,
      content: "Major central banks have been net buyers of gold, supporting long-term bullish outlook.",
    },
  ];
 ...
 */
export async function scrapeAllNews(sources: Array<{ id: string; name: string; type: string; url?: string }>): Promise<{
  ...
  console.log(`[NewsScraper] Scraping complete: ${articles.length} articles, ${events.length} events`);
